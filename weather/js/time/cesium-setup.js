...
(function(){
  ...
  window.buildEarthViewer = async function buildEarthViewer() {
    ...
    async function swapBaseForYear(year) {
      const era = year < 1800 ? 'antique' : 'modern';
      ...
      const oldLayer = era === 'antique' ? window._dayLayer : window._eraImageryProviders.antique;
      const newLayer = era === 'antique' ? window._eraImageryProviders.antique : window._dayLayer;
    ...
    try {
      if (window.Clock) {
        const applyYearToCesiumClock = (year) => {
          ...
          // BC handled by Clock.toJulianDate via ISO 8601 extended form
          let jd;
          try { jd = window.Clock.toJulianDate(year); } catch (_) { return; }
          if (!jd) return;
          // Add wall-clock seconds of today so day/night still rotates over a session
          ...
          // Era-aware base imagery swap
          if (window.swapBaseForYear) window.swapBaseForYear(year);
          // Auto-load historical layers when entering pre-modern era
